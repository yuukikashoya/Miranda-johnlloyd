...
export class AlaFBComponent implements OnInit {
  ...
  comm(value: any) {
    ...
    this.uuid = "notification" + Math.floor(100000 + Math.random() * 900000);
    set(ref(this.database, 'notification/' + this.uuid), {
      name: this.username,
      type: "comment",
      poster: this.poster
    });

    set(ref(this.database, 'notification/notify/' + this.poster), {
      poster: this.poster
    });
    this.post = "";
...
poster="";
  ...
  getComment(post: any) {
    ...
    const starCountRef2 = ref(this.database, 'post/' + this.currentpost );
    onValue(starCountRef2, (snapshot) => {
      const db2 = snapshot.val();
      this.poster = db2.name;
